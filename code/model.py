...
class Meta_Model(object):
    def __init__(self, ID_col, item_col, context_col, nb_words, model='FM',
        ...
        """
        :param ID_col: string, the column name of the item ID
        :param item_col: list, the columns of item features
        :param context_col: list, the columns of other features
        :param nb_words: dict, nb of words in each of these columns
        :param model: name of current model
        :param emb_size: embedding size of every col
        :param alpha: hyper param to balance la and lb
        :param warm_lr: lr of
        :param cold_lr: lr of local embedding
        :param ME_lr: lr of embedding generator
        ...
        def get_embeddings():
            ...
            for col in columns:
                ...
                table = tf.get_variable("table_{}".format(col), [nb_words[col], emb_size],
            ...
            # title, genres: (batch, seq_len) -> (batch, seq_len, embed_size) -> (batch, embed_size)
            inputs["title"] = tf.placeholder(tf.int32, [None, 8])
            inputs["genres"] = tf.placeholder(tf.int32, [None, 4])
            title_emb = tf.contrib.layers.embed_sequence(inputs["title"], 20001, emb_size, scope="word_emb")  # seq2seq module
            genre_emb = tf.contrib.layers.embed_sequence(inputs["genres"], 21, emb_size, scope="genre_table")
        ...
        def generate_meta_emb(item_embs):
            ...
            the pooling layer.
            """
            embs = tf.stop_gradient(tf.stack(item_embs, 1))  # 脱离计算图, 屏蔽item_embs往下的梯度计算
            item_h = tf.layers.flatten(embs)
            emb_pred_Dense = tf.layers.Dense(emb_size, activation=tf.nn.tanh, use_bias=False, name='emb_predictor')
            emb_pred = emb_pred_Dense(item_h) / 5.  # why output is devided by 5. here？？？
            ME_vars = emb_pred_Dense.trainable_variables
            return emb_pred, ME_vars

        def get_yhat_deepFM(ID_emb, item_embs, other_embs):
            """
            :return: h1 - dot(embed[i], sum(embed) - embed[i]) for i in range(len(cols)))   (batch, n_cols)
                     h2 - (batch, n_cols*emb_size) -> (batch, emb_size)
                     out - (batch, n_cols + emb_size) -> (batch, 1)

            """
            embeddings = [ID_emb] + item_embs + other_embs
            sum_of_emb = tf.add_n(embeddings)
            diff_of_emb = [sum_of_emb - x for x in embeddings]
            dot_of_emb = [tf.reduce_sum(embeddings[i] * diff_of_emb[i], axis=1, keepdims=True)
        ...
        def get_yhat_PNN(ID_emb, item_embs, other_embs):
            embeddings = [ID_emb] + item_embs + other_embs
            sum_of_emb = tf.add_n(embeddings)
            diff_of_emb = [sum_of_emb - x for x in embeddings]
            dot_of_emb = [tf.reduce_sum(embeddings[i] * diff_of_emb[i], axis=1, keepdims=True)
            ...
            b = tf.get_variable('MLP_1/bias', shape=(emb_size,), initializer=tf.initializers.zeros)
            h = tf.nn.relu(tf.matmul(h, w) + b)
            w = tf.get_variable('MLP_2/kernel', shape=(h.shape[1], 1))
            b = tf.get_variable('MLP_2/bias', shape=(1,), initializer=tf.initializers.constant(0.))
        ...
        with tf.variable_scope("model"):
            ...
            # 通过函数返回模型参数来指定模型训练参数，这招可以，说明回传的就是参数本身，不是脱离计算图的copy
        ...
        with tf.variable_scope("model", reuse=True):
            ...
            #     get the local adapted embedding, key point
        ...
        warm_update_op = warm_optimizer.minimize(warm_loss)  # update whole origin model
        warm_update_emb_op = warm_optimizer.minimize(warm_loss, var_list=[ID_table])  # only update ID embeddings
        ...
        ME_update_op = ME_optimizer.minimize(ME_loss, var_list=ME_vars)  # only update meta-learning generator

        # replace origin ID table with new generated embeddings
        ...
        def predict_ME(sess, X, Title, Genres):
            # use embedding generator to make predicion
            feed_dict = {inputs[col]: X[col] for col in columns}
            feed_dict = {inputs["title"]: Title,
                         inputs["genres"]: Genres,
                         **feed_dict}
        ...
        def train_warm(sess, X, Title, Genres, y, embedding_only=False):
            ...
            #
            feed_dict = {inputs[col]: X[col] for col in columns}
            feed_dict = {inputs["title"]: Title, inputs["genres"]: Genres, **feed_dict, label: y.reshape((-1, 1))}
            return sess.run([warm_loss, warm_update_emb_op if embedding_only else warm_update_op],
                            feed_dict=feed_dict)

        def train_ME(sess, X, Title, Genres, y, X_b, Title_b, Genres_b, y_b):
            # train the embedding generator, which needs two mini batch to calc two types of loss
            feed_dict = {inputs[col]: X[col] for col in columns}
            feed_dict = {inputs["title"]: Title, inputs["genres"]: Genres, **feed_dict, label: y.reshape((-1, 1))}
            feed_dict_b = {inputs_b[col]: X_b[col] for col in columns}
            feed_dict_b = {inputs_b["title"]: Title_b, inputs_b["genres"]: Genres_b, **feed_dict_b,
                           label_b: y_b.reshape((-1, 1))}
            return sess.run([cold_loss_a, cold_loss_b, ME_update_op],
